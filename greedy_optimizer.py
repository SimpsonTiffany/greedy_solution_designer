...
def optimize_truck_load(packages, weight_limit):
    ...
    packages.sort(key=lambda x: x["priority"] / x["weight"], reverse=True)
    ...
    for pkg in packages:
        if current_weight + pkg["weight"] <= weight_limit:
            current_weight += pkg["weight"]
            total_value += pkg["priority"]
            selected_packages.append(pkg)
        else:
            remaining = weight_limit - current_weight
            fraction = remaining / pkg["weight"]

            total_value += pkg["priority"] * fraction
            current_weight += remaining

            selected_packages.append(
                {"package_id": pkg["package_id"], "fraction": fraction}
            )
            break
...
def minimize_drivers(deliveries):
    ...
    for delivery in deliveries:
        ...
        for i in range(len(drivers)):
            if drivers[i] <= delivery["start"]:
                drivers[i] = delivery["end"]
                assignments.append((i, delivery))
                assigned = True
                break

        if not assigned:
            drivers.append(delivery["end"])
            assignments.append((len(drivers) - 1, delivery))
...
def test_truck_loading():
    ...
    print(
        f"  Got: Priority = {result1['total_priority']}, weight = {result1['total_weight']}"
    )
    ...
    print(
        f"  Got: Priority = {result2['total_priority']}, weight = {result2['total_weight']}"
    )
